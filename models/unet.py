...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        sample, init_image = self.conv_in(sample, init_image=init_image, num_frames=num_frames)
        ...
        down_block_res_init_image = (init_image,)
        for downsample_block in self.down_blocks:
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                sample, res_samples, init_image, res_init_image = downsample_block(
                ...
                sample, res_samples, init_image, res_init_image = downsample_block(hidden_states=sample, init_image=init_image, temb=emb, num_frames=num_frames)

            down_block_res_samples += res_samples
            down_block_res_init_image += res_init_image

        # 4. mid
        if self.mid_block is not None:
            sample, init_image = self.mid_block(
                sample,
                init_image,
                emb,
                encoder_hidden_states=encoder_hidden_states,
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            init_image_res_samples = down_block_res_init_image[-len(upsample_block.resnets) :]
            down_block_res_init_image = down_block_res_init_image[: -len(upsample_block.resnets)]
            ...
            if hasattr(upsample_block, "has_cross_attention") and upsample_block.has_cross_attention:
                sample, init_image = upsample_block(
                    hidden_states=sample,
                    res_hidden_states_tuple=res_samples,
                    init_image=init_image,
                    res_init_image_tuple=init_image_res_samples,
                    temb=emb,
                    encoder_hidden_states=encoder_hidden_states,
        ...
        sample, init_image = self.conv_out(sample, init_image=init_image, num_frames=num_frames)
    ...
    def from_pretrained_3d(cls, pretrained_model_path, subfolder=None):
        ...
        for k, v in model_3d.state_dict().items():
            if 'conditioning_conv' in k or 'conditioning_block' in k:
